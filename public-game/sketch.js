...
io.on('arduino-message', (arduinoMessages) => {
    console.log(socket.id);
    socket.on ('positions', (arduinoMessage) => {
        console.log(arduinoMessage);
        socket.broadcast.emit ('display-positions', arduinoMessage);   
    });
})
...
async function gameResult() {
    const req = await fetch ('/game-over');
